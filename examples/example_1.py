...
import os
import sympy
from beambending.beam import Beam, DistributedLoad, PointLoad
...
def example_1():
    ...
    beam.add_loads((
                    PointLoad(-20, 3),  # 20kN downwards, at x=3m
                    DistributedLoad(-10, (3, 9)),  # 10 kN/m, downward, for 3m <= x <= 9m
                    DistributedLoad(-20, (0, 2)),  # 20 kN/m downwards, for 0m <= x <= 2m
                ))
    fig = beam.plot()
